...
#regions = list('Tropics', 'Savanna', 'Warm Temperate', 'Cool Temperate', 'Mediterranean', 'Desert')
regions = list('trees')
i='Australia'

for (i in regions) {
  print(i)
  avhrr_path = paste('/g/data/os22/chad_tmp/climate-carbon-interactions/data/NDVI_harmonization/regions/',i,'_AVHRR_5km_monthly_1982_2013_climate.nc', sep='')
  modis_path = paste('/g/data/os22/chad_tmp/climate-carbon-interactions/data/NDVI_harmonization/regions/',i,'_MODIS_NDVI_5km_monthly_2001_2022.nc', sep='')
  ...
  tmp <- c(tmp_median, tmp_nobs, tmp_sd, tmp_sza, tmp_tod, tmp_rain_cml3, tmp_srad)
  d_cdr <- tmp %>% units::drop_units() %>% as.data.frame() %>% as.data.table()
  d_cdr <- d_cdr %>% mutate(month=month(time))
  
  #clear up some memory
  rm(tmp_median, tmp_nobs, tmp_sd, tmp_sza, tmp_tod, tmp, tmp_rain_cml3, tmp_srad)
  gc()
  ...
  dc2 <- merge(d_mcd[,.(x,y,time,ndvi_mcd)],
               d_cdr[,.(x,y,time,ndvi_cdr,sza,tod,rain_cml3,srad)], all=TRUE, 
               by=c("x","y","time"))
  ...
  dc2_train <- dc2[is.na(ndvi_mcd)==F][between(time,ymd("2001-01-01"),ymd("2013-12-31"))==T][sample(.N, 2.5e5)]
  dc2_test <- dc2[is.na(ndvi_mcd)==F][between(time,ymd("2001-01-01"),ymd("2013-12-31"))==T][sample(.N, 2.5e5)]
  ...
  stars::write_mdim(tmp,
            paste('/g/data/os22/chad_tmp/climate-carbon-interactions/data/NDVI_harmonization/regions/',i,'_climate_harmonized_NDVI_AVHRR_MODIS_1982_2013.nc', sep=''),
